...
#
# 5.7;
# 6.5; version bump: flask 2.x, flask-migrate 3.x, flask-sqlalchemy 2.5.x,
#        flask-wtf 1.x, wtforms 3.x, psycopg2 2.9.x,
#        SQLAlchemy 1.4.x, gssapi 1.7.x, eventlet 0.33.0
#        simple-websocket (not mentioned but required by werkzeug)
#      new deps : flask-babel 2.x, pyotp 2.x, qrcode 7.x, Pillow 8.3.x
#      removed : flask-babelex, itsdangerous
...
RDEPEND="${COMMON_DEPEND}
	$(python_gen_cond_dep '
		...
		=dev-python/gssapi-1.7*[${PYTHON_USEDEP}]
...
src_compile() {
	...
	yarn install --verbose --cache-folder "${WORKDIR}/yarn-cache" \
				--non-interactive --frozen-lockfile  || die
}
...
# wildcards can fail below if the folders don't exist on the actual system
	fperms 0755 /usr/share/pgadmin4/runtime/node_modules/nw/nwjs/chrome_crashpad_handler
